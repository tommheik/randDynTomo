...
%
% SYNOPSIS
%   [x, iter, out] = VMILAn_CTrandomAngles_l1(gn, R, Phi, p[, opts])
%
% MANDATORY INPUT
...
%                        DEFAULT = 1000
%   'beta'             - beta is the regularization parameter 
%                        for the regularization term, placed in front of
%                        the regularization term:
%                           min 0.5 * || R*f-y ||^2 + beta/p * ||SH(f)||_1
%                            f
%                        DEFAULT = 1
...
%    fh = figure('visible','off');
   InterRec = zeros(N2,ceil(maxit/savefreq)+1);
end
...
if flag_obj
    ...
    obj_norm = norm(obj);
    e = x - obj;
    err(1) = norm(e(:)) / obj_norm; % Note neither norm is squared!
end
...
v2 = zeros(N2,1);
fv = 0.5 * norm(x_tf-gn)^2 + beta * SHpnorm(x_sh, p); % objective function (complete: smooth + nonsmooth)
...
while loop 
    ...
    [y, Delta, v1, v2, cont] = SGP_VMILAn_CTrandomAngles_lp_NNconstraint(X, g, x, z, v1, v2, Phi, alpha, eta, beta, p, boxflag, UpBound, x_size);
    NrInnerIt(iter) = cont;  
    ...
    while fcontinue
        xplus = reshape(x + lambda*d,x_size);  % new point (try)

        x_tf_try = x_tf + lambda * d_tf;     
        
        fv = 0.5 * norm(x_tf_try-gn)^2 + beta * SHpnorm(Phi.times(xplus), p);
    ...
    switch stopcrit
        ...
        case 4
            ...
            discr = 0.5*x_size(1,1)*x_size(1,2); % What should this be actually?
